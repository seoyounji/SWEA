...
public class prob1767_Upgrade {
	...
	private static void go(int index,int cCnt,int lCnt) { //index:처리할 코어의 index , cCnt:직전까지 살펴본 코어수 , lCnt:연결된 전선의 길이
		//가지치기
		//현재까지 연결된 코어수 + 앞으로 처리해야할 남은 코어수 : 기대할 수 있는 최대코어수
		//최대코어수가 앞선 경우에서 얻은 임시해보다 작다면 그냥 그만 탐색해도 되므로 가지치기 진행
		if(cCnt+totalCnt-index < max) return;
		//최대코어수가 임시해와 같아도 일단 진행해보아야 함. 전선 길이가 달라질 수도 있으므로
		if(index==totalCnt) {
			if(max<cCnt) {
				max=cCnt;
				min=lCnt;
			} else if(max==cCnt) {
				min = Integer.min(min, lCnt);
			}
			return;
		}
		...
		for (int i = 0; i < 4; i++) {
			if(isAvailable(r,c,i)) { 
				int len = setStatus(r,c,i,2); //해당 방향으로 전선 놓는게 가능한지 체크하고 가능하다면 전선 놓기 : 맥시노스판에 2로 입력
	...
	//현코어의 위치에서 해당 방향으로 전선을 놓거나(2) 지우는(0) 함수 + 놓아진 전선의 길이를 세는 함수
	private static int setStatus(int r,int c,int d,int value) {
		int nr=r,nc=c,cnt=0;
		while(true) {
			nr += dx[d];
			nc += dy[d];
